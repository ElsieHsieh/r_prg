...
filter_del_franky <- straw_hat_df$name != "Franky" #== 刪除franky之外的，!=就是只刪除一個

straw_hat_df <- straw_hat_df[filter_del_franky,]

# write.csv(straw_hat_df, file = "MyData.csv",row.names=TRUE)

# subset(straw_hat_df,subset=gender=="Male",select=c(name,age))

birth_year <- as.numeric(format(Sys.Date(),format="%Y"))-as.numeric(straw_hat_df$age)
#Sys.Date()%>%format(format="%Y")%>%as.numeric() #兩個方法一樣，美觀與易讀性不同
birth_date_chr <- paste(birth_year,straw_hat_df$birthday,sep = "-")
as.Date(birth_date_chr)

# as.Date(date_format_demo,format="%B,%d,%y") 先指定原本格式去轉